...
Analizando la captura podemos ver que el dominio se llama `sequel.htb` y que el controlador de dominio es `dc01.sequel.htb`, agregaremos esta información al archivo `/etc/hosts` con el siguiente comando
...
cat xl/sharedStrings.xml | grep -oP '<t xml:space="preserve">\K[^<]+' | grep -vE "First Name|Last Name|Password|Email|Username" | paste - - - - - | column -s $'\t' -t > accounts_table.txt
~~~
